...
func (p *paddingConn) readWithPadding(reader io.Reader, buffer []byte) (n int, err error) {
	...
	if p.readPadding < naivePaddingCount {
...
func (p *paddingConn) writeWithPadding(writer io.Writer, data []byte) (n int, err error) {
	if p.writePadding < naivePaddingCount {
...
func (p *paddingConn) readerReplaceable() bool { return p.readPadding == naivePaddingCount }
func (p *paddingConn) writerReplaceable() bool { return p.writePadding == naivePaddingCount }

// ─── naiveConn (TCP / hijacked HTTP/1.1) ─────────────────────────────────────
...
func (c *naiveConn) WriteBuffer(buffer *buf.Buffer) error {
	defer buffer.Release()
	return naiveWrapError(c.writeBufferWithPadding(c.Conn, buffer))
}
...
// ─── naiveH2Conn (HTTP/2 and HTTP/3 streams) ─────────────────────────────────
...
func (c *naiveH2Conn) WriteBuffer(buffer *buf.Buffer) error {
	...
	return naiveWrapError(err)
}

func (c *naiveH2Conn) Close() error                    { return common.Close(c.reader, c.writer) }
func (c *naiveH2Conn) LocalAddr() net.Addr             { return M.Socksaddr{} }
func (c *naiveH2Conn) RemoteAddr() net.Addr            { return c.remoteAddress }
func (c *naiveH2Conn) SetDeadline(time.Time) error     { return os.ErrInvalid }
func (c *naiveH2Conn) SetReadDeadline(time.Time) error { return os.ErrInvalid }
func (c *naiveH2Conn) SetWriteDeadline(time.Time) error { return os.ErrInvalid }
...
func naiveWrapError(err error) error {
	err = baderror.WrapH2(err)
	if NaiveWrapError != nil {
		err = NaiveWrapError(err)
	}
	return err
}
